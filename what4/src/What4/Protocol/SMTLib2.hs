...
getSymFnValue :: (SMTLib2Tweaks a, I.IsSymExprBuilder sym) => Session t a -> sym -> Text -> IO (I.SomeSymFn sym)
getSymFnValue s sym nm = (parseLambda sym nm) =<< runGetValue s (fromText nm)

parseLambda :: I.IsSymExprBuilder sym => sym -> Text -> SExp -> IO (I.SomeSymFn sym)
parseLambda sym nm sexp = case sexp of
  SApp ["lambda", SApp params_sexp, body_sexp] -> do
    (nms, vars) <- unzip <$> mapM (parseVar sym) params_sexp
    Some vars_assign <- return $ Ctx.fromList vars
...
parseType sexp = case sexp of
  SAtom "Bool" -> return $ Some BaseBoolRepr
  SAtom "Int" -> return $ Some BaseIntegerRepr
  SApp ["_", SAtom "BitVec", SAtom (Text.unpack -> m_str)]
    | [(m_n, "")] <- readDec m_str
...
parseExpr sym sexp = do
 liftIO $ putStrLn $ "Parsing " ++ show sexp
 case sexp of
  SAtom "true" -> return $ Some $ I.truePred sym
  SAtom "false" -> return $ Some $ I.falsePred sym
...
readThreeArgs ::
  ...
  => sym
  -> [SExp]
  -> Processor sym (Some (I.SymExpr sym), Some (I.SymExpr sym), Some (I.SymExpr sym))
readThreeArgs sym operands = do
  args <- mapM (parseExpr sym) operands
  case args of
    [arg1, arg2, arg3] -> return (arg1, arg2, arg3)
    _ -> throwError $ printf "expecting 3 arguments, got %d" (length args)

...
class (SupportTermOps (Term h)) => SMTWriter h where
  ...
  ffNeg :: (2 <= p) => NatRepr p -> Term h -> Term h
...
appSMTExpr ae = do
  ...
  case appExprApp ae of
        ...
        SR.SemiRingBVRepr SR.BVBitsRepr w ->
          do pd' <- WSum.prodEvalM (\a b -> pure (bvAnd a b)) mkBaseExpr pd
             maybe (return $ SMTExpr (BVTypeMap w) $ bvTerm w (BV.maxUnsigned w))
                   ...
                   pd'
        SR.SemiRingFFRepr p ->
          do pd' <- WSum.prodEvalM (\a b -> pure (ffMul a b)) mkBaseExpr pd
             maybe (return $ SMTExpr (FFTypeMap p) $ ffTerm p (mkFin (knownNat @1)))
                   (freshBoundTerm (FFTypeMap p))
                   pd'
        sr ->
    ...
    FFNeg p x -> do
      xe <- mkBaseExpr x
      freshBoundTerm (FFTypeMap p) $ ffNeg @h p xe
    FFRecip p x -> do
      xe <- mkBaseExpr x
      val <- asBase <$> freshConstant "ff_recip" (FFTypeMap p)
      -- (assert (= (ff.mul val val xe) val))
      addSideCondition "ff_binary" $ ffMul (ffMul val val) xe .== val
      -- (assert (= (ff.mul val xe xe) xe))
      addSideCondition "ff_binary" $ ffMul (ffMul val xe) xe .== xe
      freshBoundTerm (FFTypeMap p) val

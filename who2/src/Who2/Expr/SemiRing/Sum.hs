...
instance (PC.TestEquality f, Eq (f (SR.SemiRingBase sr))) => Eq (SRSum sr f) where
  ws1 == ws2 =
    SR.eq (sumRepr ws1) (sumOffset ws1) (sumOffset ws2)
    P.&& BKv.eqBy (==) (SR.eq (sumRepr ws1)) (sumMap ws1) (sumMap ws2)

instance (PC.TestEquality f, Ord (f (SR.SemiRingBase sr)), Ord (SR.Coefficient sr)) => Ord (SRSum sr f) where
  compare ws1 ws2 =
    case P.compare (sumOffset ws1) (sumOffset ws2) of
      P.EQ -> BKv.ordBy compare (SR.sr_compare (sumRepr ws1)) (sumMap ws1) (sumMap ws2)
      c -> c

instance (PC.TestEquality f, PC.HashableF f, Hashable (f (SR.SemiRingBase sr)), Hashable (SR.Coefficient sr)) => Hashable (SRSum sr f) where
  hashWithSalt s ws =
    s `hashWithSalt` sumOffset ws `hashWithSalt` sumMap ws
